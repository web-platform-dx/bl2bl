...
import('baseline-browser-mapping').then(bbm => {
    ...
    if (
        ...
        baselineVersions = bbm.getMinimumWidelyAvailable(packageJSON.bl2bl.downstreamBrowsers);
    } else if ((
        ...
        baselineVersions = bbm.getMinimumByYear(bl2blConfig.baselineThreshold, bl2blConfig.downstreamBrowsers);
    }
    else {
    ...
    baselineVersions.forEach(browser => {
        ...
        if (Object.keys(browsers).includes(browser.browser)) {
            ...
            browserslistOutput.push(`${browsers[browser.browser]} >= ${browser.version}`)
        }
    });
    ...
    if (packageJSON != null) {
        fs.writeFileSync(process.cwd() + '/package.json', JSON.stringify(packageJSON, null, 2), { encoding: 'utf8' });
    }
